					...
					if(contours2[i].size() >=10){
						...
						{	
							if( error < .8)
							{
								...
								int radius = rec.size.width/2;
								double black_sum = 0;
								double total = 0;
								for(int j = center.x-rec.size.width/2; j < center.x + rec.size.width/2; j++)
								for(int k = center.y-rec.size.height/2; k <center.y +rec.size.height/2; k++)
								{
									int a = j-center.x;
									int b = k-center.y;
									...
									{
										//circle(thresh_image,Point(j,k),1,Scalar(0,200,0),1,8,0);
										total++;
										
										if(thresh_image.at<uchar>(j, k) == 0)
											black_sum++;
									}
								}
